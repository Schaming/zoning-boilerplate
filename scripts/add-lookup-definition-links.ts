// scripts/add-lookup-definition-links.ts
// Walks bylawSubsections rich text blocks and wraps matching terms in link nodes
...
async function fetchAllDefinitions(payload: Payload): Promise<TermAlias[]> {
  ...
  while (true) {
    ...
    res.docs.forEach(doc => {
      ...
      if (termStr) {
        aliases.push({ alias: termStr, canonical: termStr });
      }
    });
...
const aliasLookup = (aliases: TermAlias[]) => {
  ...
  aliases.forEach(a => map.set(a.alias.toLowerCase(), a.canonical));
  return map;
};
...
function wrapTextNodeIfMatches(
  ...
  for (const match of text.matchAll(regex)) {
    ...
    const canonical = aliasMap.get(baseTerm.toLowerCase()) || baseTerm;
...
function processNode(
  ...
  if (node.type === 'link') {
    if (node.fields?.url === '#' && !node.fields?.actionKey) {
      ...
      if (match) {
        ...
        const canonical = aliasMap.get(baseTerm.toLowerCase()) || baseTerm;
        console.log(`Upgrading manual link: "${text}" -> "${canonical}"`);
        return {
          changed: true,
          nodes: [
            {
  ...
  if (node.type === 'text') {
    const wrapped = wrapTextNodeIfMatches(node, regex, aliasMap);
  ...
  if (Array.isArray(node.children)) {
    ...
    node.children.forEach(child => {
      const res = processNode(child, regex, aliasMap);
...
function processRichText(body: LexicalRoot, regex: RegExp, aliasMap: Map<string, string>) {
...
async function processSubsections(payload: Payload, regex: RegExp, aliasMap: Map<string, string>) {
  let page = 1;
  let updated = 0;
  while (true) {
    const res = await payload.find({
    ...
    for (const doc of res.docs) {
      ...
      const newContent = content.map((block: any) => {
        ...
        const { changed, body: newBody } = processRichText(body, regex, aliasMap);
        if (changed) changedBlocks = true;
        return changed ? { ...block, body: newBody } : block;
      });
      ...
      if (changedBlocks) {
        try {
          ...
          updated += 1;
          console.log(`Updated bylawSubsection ${doc.id}`);
        } catch (err: any) {
          console.error('Failed to update bylawSubsection', doc.id);
          if (err?.data) {
  ...
  console.log(`Finished. Updated ${updated} subsections.`);
}
...
async function start() {
  ...
  await processSubsections(payload, regex, aliasMap);
}
